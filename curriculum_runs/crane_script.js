...
// HTML escape for any string interpolated into innerHTML. Crane id/type and lift id
// flow through the scenario editor and imported JSON, both of which can contain
// arbitrary characters — escaping them prevents stored XSS.
function esc(s){return String(s==null?'':s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}

// ============================================================================
// Sections (search for "// =====" to jump between them)
//   Config & model I/O .................. ~ line 1
//   RNG, geometry, radius primitives ..... ~ line 60
//   Browser-light MAPPO (actor/critic) ... ~ line 65
//   Scenario / state / candidates ........ ~ line 200
//   Baseline policies + masking .......... ~ line 270
//   makePlan, enforceMaskedChoices, runEpisode ~ line 295
//   Training drivers + curriculum ........ ~ line 430
//   Planning UI + import/export .......... ~ line 535
//   Validation & sensitivity ............. ~ line 678
//   Renderers (innerHTML, summary) ....... ~ line 760
//   Replay & canvas drawing .............. ~ line 770
...
function compactEval(r){
  if(!r)return null;
  return {done:r.done,total:r.total,makespan:r.makespan,reward:r.reward,softInter:r.softInter||0,hardExecuted:r.hardExecuted||0,hardMask:r.hardMask??0,interPenaltyTotal:r.interPenaltyTotal||0,same:r.same,setupTotal:r.setupTotal,travelTotal:r.travelTotal,liftTotal:r.liftTotal,moveTotal:r.moveTotal,craneFinish:(r.cranes||[]).map(c=>({id:c.id,done:c.done||0,finish:Math.round(c.availableTime||0),reward:c.reward||0,moveDistance:c.moveDistance||0}))};
}
...
// ===== RNG, geometry & radius primitives =====================================
...
// ===== Browser-light MAPPO (actor/critic + update) ===========================
...
function candidateOutcome(cr,lift,cranesRun=[],liftsRun=[]){
  // crane_speed=10 (distance units per minute) — matches python_mappo/config.yaml so
  // browser-light and PyTorch makespans are directly comparable.
  const c=cfg(), CRANE_SPEED=10, sp=setupPoint(cr), same=lift?inCraneRadius(cr,lift):false, target=lift?setupTargetForLift(sp,lift,c):sp, moveDist=lift?(same?0:dist(sp,target)):0, travel=moveDist/CRANE_SPEED, setup=lift?(same?0:c.setupTime):0;
...
// NOTE on naming: this is the browser-light update — a single-step
// advantage-weighted policy gradient with subtracted expected-feature baseline,
// NOT real PPO. ratio is hard-coded to 1 so the ppoClip widget is decorative
// on this path. PyTorch MAPPO in python_mappo/mappo.py is the canonical PPO
// implementation; this lightweight pass exists for in-browser visualization.
...
// ===== Scenario / state / candidate generation ===============================
...
// ===== Plan, mask enforcement, runEpisode ====================================
function makePlan(ch,cs,ls){
  ...
  // crane_speed=10 — see candidateOutcome.
  let c=cfg(), CRANE_SPEED=10, sp=setupPoint(cr), sameSetup=inCraneRadius(cr,lift), target=setupTargetForLift(sp,lift,c), moveDist=sameSetup?0:dist(sp,target), travel=moveDist/CRANE_SPEED, setup=sameSetup?0:c.setupTime, start=cr.availableTime, finish=start+travel+setup+lift.duration;
...
function runEpisode(ep=0, learn=true, epsilon=0.1, policy='rl'){const c=cfg(); let sc=cloneScenario(); let cs=sc.cranes, ls=sc.lifts; let totalReward=0, inter=0, softInter=0, hardMaskCount=0, interPenaltyTotal=0, sameCount=0, setupTotal=0, travelTotal=0, liftTotal=0, moveTotal=0, events=[];
  ...
  for(let step=0; step<c.maxSteps && ls.some(l=>l.status==='todo'); step++){
    ...
    const masked=enforceMaskedChoices(choices,policy,cs,ls,events); choices=masked.choices; hardMaskCount+=masked.hardMasked;
  ...
  let done=ls.filter(l=>l.status==='done').length, makespan=Math.max(0,...cs.map(c=>c.availableTime||0),...ls.map(l=>l.finish||0));
  // Average actual lifting / danger radius across recorded events. The browser
  // used to leave these as 0, which made fair_compare's radius columns
  // misleading; populating them matches the PyTorch env summary contract.
  const sb=softBufferValue();
  const actualLiftRadiusAvg = events.length ? events.reduce((a,e)=>a+(e.actualLiftRadius||0),0)/events.length : 0;
  const actualDangerRadiusAvg = events.length ? events.reduce((a,e)=>a+((e.actualLiftRadius||0)+sb),0)/events.length : 0;
  return {reward:totalReward, done, total:ls.length, makespan:Math.round(makespan), inter, softInter, hardExecuted:0, hardMask:hardMaskCount, interPenaltyTotal, same:sameCount, setupTotal, travelTotal, liftTotal, moveTotal, actualLiftRadiusAvg, actualDangerRadiusAvg, cranes:cs, lifts:ls, events};}
// ===== Training drivers (single / multi / curriculum) ========================
...
async function trainMappoCurriculumModel(stages=curriculumStages(), preserveUi=false){
  ...
  for(const st of stages){
    ...
    renderCurriculumRows(stageResults,{makespan:0,makespanSd:0,softInter:0,hardMask:0,travel:0,setup:0},{makespan:0,makespanSd:0,softInter:0,hardMask:0,travel:0,setup:0});
    await uiYield();
  }
...
function renderCurriculumRows(stageResults,stdStats,hardStats){$('repeatRows').innerHTML=[...stageResults.map(s=>`<tr><td>${esc(s.stage)} 학습</td><td>${s.nC} cranes / ${s.nL} lifts</td><td>${s.episodes} ep, seed ${s.seedRuns}</td><td>완료율 ${s.avgComplete.toFixed(1)}%</td><td>best ${s.bestMakespan??'-'}분 / K=${s.candidateK}</td></tr>`),`<tr><td>표준 평가</td><td>3 cranes / 24 lifts</td><td>${stdStats.makespan.toFixed(1)}±${stdStats.makespanSd.toFixed(1)}분</td><td>Soft ${stdStats.softInter.toFixed(1)} / HardMask ${stdStats.hardMask.toFixed(1)}</td><td>Travel ${stdStats.travel.toFixed(1)} / Setup ${stdStats.setup.toFixed(1)}</td></tr>`,`<tr><td>확장 평가</td><td>4 cranes / 36 lifts</td><td>${hardStats.makespan.toFixed(1)}±${hardStats.makespanSd.toFixed(1)}분</td><td>Soft ${hardStats.softInter.toFixed(1)} / HardMask ${hardStats.hardMask.toFixed(1)}</td><td>Travel ${hardStats.travel.toFixed(1)} / Setup ${hardStats.setup.toFixed(1)}</td></tr>`].join('');}
function renderCurriculumInterpretation(stageResults,stdStats,hardStats){$('interpretLog').textContent=[`Curriculum 1→2→3 학습 완료`,`- 1단계: 2 cranes / 12 lifts로 기본 작업 완료·setup 유지·idle 회피를 먼저 학습`,`- 2단계: 3 cranes / 24 lifts로 현재 표준 Stage-1.5 조건에 적응`,`- 3단계: 4 cranes / 36 lifts로 더 많은 크레인/양중물 및 실제 양중반경 간섭 조합에 적응`,`- 표준 평가 평균 makespan: ${stdStats.makespan.toFixed(1)}분, Soft ${stdStats.softInter.toFixed(1)}, HardMask ${stdStats.hardMask.toFixed(1)}`,`- 확장 평가 평균 makespan: ${hardStats.makespan.toFixed(1)}분, Soft ${hardStats.softInter.toFixed(1)}, HardMask ${hardStats.hardMask.toFixed(1)}`,`- 해석: actor/critic weight를 유지한 채 난이도만 올렸으므로, 단순 재학습이 아니라 curriculum learning 결과입니다.`].join('\n');}
...
function renderPlanningSummary(r,label='계획안'){
  ...
  $('pMakespan').textContent=`${r.makespan}분`; $('pDone').textContent=`${donePct}%`; $('pInter').textContent=`S ${r.softInter||0} / EH ${r.hardExecuted||0} / M ${r.hardMask??0}`; $('pPlanTime').textContent=`${r.travelTotal.toFixed(1)} / ${r.setupTotal.toFixed(1)}`; $('pMoveMetric').textContent=r.moveTotal.toFixed(1);
  $('planningLog').textContent=[`${label} 생성 완료`,`- 완료: ${r.done}/${r.total} (${donePct}%)`,`- Makespan: ${r.makespan}분, reward ${r.reward.toFixed(1)}`,`- Soft 노출: ${r.softInter||0}회, 실행 Hard overlap: ${r.hardExecuted||0}회, HardMask 차단: ${r.hardMask??0}회`,`- Travel ${r.travelTotal.toFixed(1)}분 / Setup ${r.setupTotal.toFixed(1)}분 / Move ${r.moveTotal.toFixed(1)}`,`- 평균 실제 양중반경: ${rad.lift.toFixed(1)}, 평균 실제 위험반경: ${rad.danger.toFixed(1)}`,`- 실제 양중반경/위험반경 중심은 crane/setup point입니다. Hard overlap은 실행되지 않고 action mask로 차단됩니다.`].join('\n');
}
...
function renderPlanningGantt(r){
  ...
  $('planningGantt').innerHTML=r.cranes.map((cr,i)=>{
    const evs=(r.events||[]).filter(e=>e.craneId===cr.id).sort((a,b)=>a.start-b.start), bars=evs.map(e=>{
      ...
      return `<div class="gantt-bar" title="${esc(e.craneId)} ${esc(e.liftId)}: ${Math.round(e.start)}~${Math.round(e.finish)}분" style="left:${left}%;width:${width}%;background:${color}">${esc(e.liftId)}</div>`;
    }).join('');
    return `<div class="gantt-row"><b>${esc(cr.id)}</b><div class="gantt-line">${bars}</div></div>`;
  }).join('')+`<div class="note">0 ~ ${maxT}분 · 노랑/주황 bar는 soft 노출 포함 작업입니다.</div>`;
}
function renderPlanningCompareRows(results){
  $('planningCompareRows').innerHTML=results.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.r.done}/${x.r.total}</td><td>${x.r.makespan}분</td><td>${x.r.reward.toFixed(1)}</td><td>Soft ${x.r.softInter||0} / 실행Hard ${x.r.hardExecuted||0} / HardMask ${x.r.hardMask??0}</td><td>${x.r.travelTotal.toFixed(1)}</td><td>${x.r.setupTotal.toFixed(1)}</td><td>${x.r.moveTotal.toFixed(1)}</td></tr>`).join('');
}
...
function pySummaryToResult(s,total=cfg().nL){return {done:Math.round((s.completeRate||0)/100*total),total,makespan:+(s.makespan||0).toFixed(2),reward:s.reward||0,softInter:s.soft||0,hardExecuted:s.hardExecuted||0,hardMask:s.hardMask||0,travelTotal:s.travel||0,setupTotal:s.setup||0,moveTotal:s.move||0,liftTotal:total*cfg().fixedDuration,same:0,cranes:[],events:[]};}
...
function summarizePolicy(samples,wins,total){const vals=k=>samples.map(s=>s[k]??0); const avg=a=>a.reduce((x,y)=>x+y,0)/Math.max(1,a.length); const sd=a=>{let m=avg(a); return Math.sqrt(avg(a.map(x=>(x-m)*(x-m))));}; return {n:samples.length,makespan:avg(vals('makespan')),makespanSd:sd(vals('makespan')),reward:avg(vals('reward')),rewardSd:sd(vals('reward')),inter:avg(vals('inter')),softInter:avg(vals('softInter')),hardMask:avg(vals('hardMask')),interPenalty:avg(vals('interPenaltyTotal')),travel:avg(vals('travelTotal')),setup:avg(vals('setupTotal')),move:avg(vals('moveTotal')),winRate:wins/Math.max(1,total)};}
...
function renderSeenUnseenRows(summary){$('repeatRows').innerHTML=summary.map(x=>{let gap=x.unseen.makespan-x.seen.makespan, pct=gap/Math.max(1,x.seen.makespan)*100; return `<tr><td>${esc(x.name)}</td><td>Seen ${x.seen.makespan.toFixed(1)}분</td><td>Unseen ${x.unseen.makespan.toFixed(1)}분</td><td>${gap>=0?'+':''}${gap.toFixed(1)}분 (${pct.toFixed(1)}%)</td><td>Unseen Soft ${x.unseen.softInter.toFixed(1)} / HardMask ${x.unseen.hardMask.toFixed(1)}</td></tr>`;}).join('');}
...
function renderSensitivityRows(rows){$('repeatRows').innerHTML=rows.map(x=>`<tr><td>pSoft=${x.p}</td><td>${x.stats.makespan.toFixed(1)}±${x.stats.makespanSd.toFixed(1)}분</td><td>${x.stats.reward.toFixed(1)}±${x.stats.rewardSd.toFixed(1)}</td><td>Soft ${x.stats.softInter.toFixed(1)} / HardMask ${x.stats.hardMask.toFixed(1)}</td><td>Travel ${x.stats.travel.toFixed(1)} / Setup ${x.stats.setup.toFixed(1)} / Move ${x.stats.move.toFixed(1)}</td></tr>`).join('');}
...
function renderRepeatRows(summary){$('repeatRows').innerHTML=summary.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.stats.makespan.toFixed(1)}±${x.stats.makespanSd.toFixed(1)}분</td><td>${x.stats.reward.toFixed(1)}±${x.stats.rewardSd.toFixed(1)}</td><td>${x.stats.inter.toFixed(1)}회 (Soft ${x.stats.softInter.toFixed(1)} / HardMask ${x.stats.hardMask.toFixed(1)})</td><td>${(x.stats.winRate*100).toFixed(1)}%</td></tr>`).join('');}
...
// ===== Renderers (innerHTML, summary cards) ==================================
function renderSummaryMetrics(r){$('mEvalMakespan').textContent=r?r.makespan+'분':'-'; $('mInter').textContent=r?`Soft ${r.softInter||0}회 / HardMask ${r.hardMask||0}회`:'-'; $('mTravel').textContent=r?r.travelTotal.toFixed(1)+'분':'-'; $('mSetup').textContent=r?r.setupTotal.toFixed(1)+'분':'-'; if($('mMove'))$('mMove').textContent=r?r.moveTotal.toFixed(1):'-';}
function renderBaselineRows(results){$('baselineRows').innerHTML=results.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.r.done}/${x.r.total}</td><td>${x.r.makespan}분</td><td>${x.r.reward.toFixed(1)}</td><td>Soft ${x.r.softInter||0} / 실행Hard ${x.r.hardExecuted||0} / HardMask ${x.r.hardMask??0}</td><td>${x.r.travelTotal.toFixed(1)}</td><td>${x.r.setupTotal.toFixed(1)}</td><td>${x.r.moveTotal.toFixed(1)}</td></tr>`).join('');}
function renderMetricLog(data,title){
  if(Array.isArray(data)){$('metricLog').textContent=`${title}\n`+data.map(x=>`${x.name}: 완료 ${x.r.done}/${x.r.total}, makespan ${x.r.makespan}분, reward ${x.r.reward.toFixed(1)}, soft노출 ${x.r.softInter||0}회 / 실행Hard ${x.r.hardExecuted||0}회 / hard마스크 ${x.r.hardMask??0}회, soft penalty ${(x.r.interPenaltyTotal||0).toFixed(1)}, 동일 반경 연속 ${x.r.same}회, travel ${x.r.travelTotal.toFixed(1)}분, 이동거리 ${x.r.moveTotal.toFixed(1)}, setup ${x.r.setupTotal.toFixed(1)}분, lift ${x.r.liftTotal.toFixed(1)}분`).join('\n')+`\n\n해석 기준: 실제 양중반경 hard overlap은 action mask로 실행 차단됩니다. Soft는 허용 가능한 안전/영향 반경 근접 노출이므로 실패 지표가 아니라 보조 지표로 봅니다.`; return;}
...
Hard action mask: ${data.hardMask??0}회, 실제 양중반경 overlap 후보 차단\n- Soft penalty 합계: ${(data.interPenaltyTotal||0).toFixed(1)}\n동일 반경 연속: ${data.same}회\nTravel: ${data.travelTotal.toFixed(1)}분\n이동거리: ${data.moveTotal.toFixed(1)}\nSetup: ${data.setupTotal.toFixed(1)}분\nLift: ${data.liftTotal.toFixed(1)}분\n크레인별 완료시각: ${data.cranes.map(c=>`${c.id} ${Math.round(c.availableTime)}분`).join(', ')}\n\n평가 우선순위: makespan → 완료율/HardMask 정상 작동 → travel/setup/move 효율 → soft 노출 → reward`;
}
function renderAgents(r){let rows=''; let cs=(r?r.cranes:cranes); cs.forEach(c=>rows+=`<tr><td>${esc(c.id)}<br><span class="note">${esc(c.type||'50톤 모바일 크레인')}</span></td><td>${c.done||0}</td><td>${Math.round(c.availableTime||0)}분</td><td>${Math.round(c.idle||0)}분</td><td>${(c.reward||0).toFixed(1)}</td><td>${(c.moveDistance||0).toFixed(1)}</td></tr>`); $('agentRows').innerHTML=rows;}
