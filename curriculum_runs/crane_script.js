...
function runEpisode(ep=0, learn=true, epsilon=0.1, policy='rl'){const c=cfg(); let sc=cloneScenario(); let cs=sc.cranes, ls=sc.lifts; let totalReward=0, inter=0, softInter=0, hardInter=0, interPenaltyTotal=0, sameCount=0, setupTotal=0, travelTotal=0, liftTotal=0, moveTotal=0, events=[];
  ...
  return {reward:totalReward, done, total:ls.length, makespan:Math.round(makespan), inter, softInter, hardExecuted:0, hardMask:hardInter, hardInter, interPenaltyTotal, same:sameCount, setupTotal, travelTotal, liftTotal, moveTotal, actualLiftRadiusAvg, actualDangerRadiusAvg, cranes:cs, lifts:ls, events};}
...
function renderPlanningCompareRows(results){
  $('planningCompareRows').innerHTML=results.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.r.done}/${x.r.total}</td><td>${x.r.makespan}분</td><td>${x.r.reward.toFixed(1)}</td><td>Soft ${x.r.softInter||0} / 실행Hard ${x.r.hardExecuted||0} / HardMask ${x.r.hardMask??x.r.hardInter??0}</td><td>${x.r.travelTotal.toFixed(1)}</td><td>${x.r.setupTotal.toFixed(1)}</td><td>${x.r.moveTotal.toFixed(1)}</td></tr>`).join('');
}
...
function renderBaselineRows(results){$('baselineRows').innerHTML=results.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.r.done}/${x.r.total}</td><td>${x.r.makespan}분</td><td>${x.r.reward.toFixed(1)}</td><td>Soft ${x.r.softInter||0} / 실행Hard ${x.r.hardExecuted||0} / HardMask ${x.r.hardMask??x.r.hardInter??0}</td><td>${x.r.travelTotal.toFixed(1)}</td><td>${x.r.setupTotal.toFixed(1)}</td><td>${x.r.moveTotal.toFixed(1)}</td></tr>`).join('');}
